    ...
    Tagify.prototype = {
        ...
        events: {
            ...
            callbacks: {
                ...
                onPaste: function onPaste(e) {
                    ...
                    if (this.settings.mode == 'mix') this.injectAtCaret(pastedData, window.getSelection());
                    else this.input.set.call(this, pastedData);
                },
        ...
        reCheckInvalidTags: function reCheckInvalidTags() {
            ...
            [].forEach.call(tagElms, function(node) {
                ...
                if (wasNodeDuplicate && isNodeValid) {
                    if (tagData.__preInvalidData) tagData = tagData.__preInvalidData;
                    else // start fresh
                        tagData = {
                        value: tagData.value
        ...
        removeTags: function removeTags(tagElms, silent, tranDuration) {
            ...
            this.settings.hooks.beforeRemoveTag(tagsToRemove).then(function() {
                ...
                if (tranDuration && tranDuration > 10 && tagsToRemove.length == 1) animation.call(_this12, tagsToRemove[0]);
                else tagsToRemove.forEach(removeNode.bind(_this12)); // update state regardless of animation
                ...
                rootNode.childNodes.forEach(function(node) {
                    if (node.nodeType == 1) {
                        if (node.classList.contains("tagify__tag") && that.tagData(node)) {
                            ...
                            else result += _interpolator[0] + JSON.stringify(node.__tagifyTagData) + _interpolator[1];
                            return;
                        }
